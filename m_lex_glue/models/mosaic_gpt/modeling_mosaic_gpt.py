...
NOTE when/if this gets integrated into mosaicml/examples, we should use a single source of truth for this

This version is from the mosaicGPT repo, where I am attempting to make this a HF compatible model
"""
...
from m_lex_glue.models.mosaic_gpt.configuration_mosaic_gpt import MosaicGPTConfig
from m_lex_glue.models.mosaic_gpt.flash_attention import FlashMHA
...
class TritonFlashCausalAttention(nn.Module):
    ...
    def __init__(self, cfg: DictConfig, device: Optional[str] = None):
        super().__init__()
        try:
            from m_lex_glue.models.mosaic_gpt.flash_attention import FlashMHA  # type: ignore
        except ImportError as e:
            raise e
...
# this is called MosaicGPT in the source, examples/llm
...
class ComposerMosaicGPT(ComposerModel):
    ...
    def eval_forward(self, batch, outputs: Optional[Any] = None):
        ...
        if mode == "clm":
            ...
            # need to tweak decoder hyperparams
...
class MosaicGPTForCausalLM(PreTrainedModel):
    config_class = MosaicGPTConfig

    def __init__(self, config):
        # config could be a HF config or OM config:
        try:
            super().__init__(MosaicGPTConfig(**config))
        except:
            # HF config needs to be omegaconf
            super().__init__(config)
            config = config.to_dict()
            config = om.create(config)
        self.model = MGPT(config)
    ...
    def prepare_inputs_for_generation(self, input_ids, past=None, **kwargs):
        ...
        if attention_mask is None:
            attention_mask = kwargs.get("key_padding_mask", None)
